...
exports.route = function(app, basePath, middleware, module){
	...
	switch(req.method){
	case 'GET':  // fetch id
            if (checkAndCall(module, "getById", req.param('id'), req, res)){
                return;
            }
            break;
	case 'PUT':  // update id
            if (checkAndCall(module, "updateById", req.param('id'), req, res)){
                return;
            }
            break;
	case 'POST': // create id
	...
	case 'DELETE': // delete id
            if (checkAndCall(module, "deleteById", req.param('id'), req, res)){
                return;
            }
            break;
	default:
	...
	case 'PUT':
	    break;
	case 'POST':
            if (checkAndCall(module, "create", req, res)){
                return;
            }
            break;
	case 'DELETE':
